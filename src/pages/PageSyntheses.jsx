...
// ── Single chapter row ───────────────────────────────────────────────────────
function ChapterRow({ chap, idx, onSetStatus, onEditName, onEditNote }) {
  const { t } = useTranslation();
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * .03 }}
      style={{ background: 'var(--bg-card)', border: `1px solid var(--border)`,
        borderLeft: `2px solid ${s.color}`, borderRadius: 10, overflow: 'hidden', transition: 'border-color .2s' }}>
...
function SubjectCard({ subject, filter, onUpdate, index }) {
  const { t } = useTranslation();
